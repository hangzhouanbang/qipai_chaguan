...
import com.anbang.qipai.chaguan.msg.channel.sink.DoudizhuGameSink;
...
public class DoudizhuGameMsgReceiver {
	...
	public void receive(CommonMO mo) {
		...
		if ("playerQuit".equals(msg)) {// 有人退出游戏
			...
			GameTable table = gameService.findTableByGameAndServerGameGameId(Game.doudizhu, gameId);
			if (table != null) {
				gameService.gamePlayerQuitQame(Game.doudizhu, gameId, playerId);
		...
		if ("ju canceled".equals(msg)) {// 取消游戏
			Map data = (Map) mo.getData();
			String gameId = (String) data.get("gameId");
			GameTable gameTable = gameService.findTableByGameAndServerGameGameId(Game.doudizhu, gameId);
			if (gameTable != null) {
				gameTableCmdService.removeTable(gameTable.getNo());
				gameService.gameTableFinished(Game.doudizhu, gameId);
		...
		if ("game start".equals(msg)) {// 游戏开始
			...
			GameTable gameTable = gameService.findTableByGameAndServerGameGameId(Game.doudizhu, gameId);
			if (gameTable != null) {
				gameService.gameTablePlaying(Game.doudizhu, gameId);
				gameTableMsgService.updateGameTable(gameTable);
			}
		}
		if ("game delay".equals(msg)) {// 游戏延时
			...
			GameTable gameTable = gameService.findTableByGameAndServerGameGameId(Game.doudizhu, gameId);
			// 延时1小时
			gameService.delayGameTable(Game.doudizhu, gameId, gameTable.getDeadlineTime() + 1 * 60 * 60 * 1000);
		}
	}
}
