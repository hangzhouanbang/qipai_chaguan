...
import com.anbang.qipai.chaguan.msg.channel.sink.DaboluoGameSink;
...
public class DaboluoGameMsgReceiver {
	...
	public void receive(CommonMO mo) {
		...
		if ("playerQuit".equals(msg)) {// 有人退出游戏
			...
			if (table != null) {
				gameService.gamePlayerQuitQame(Game.daboluo, gameId, playerId);
		...
		if ("ju canceled".equals(msg)) {// 取消游戏
			Map data = (Map) mo.getData();
			String gameId = (String) data.get("gameId");
			GameTable gameTable = gameService.findTableByGameAndServerGameGameId(Game.daboluo, gameId);
			if (gameTable != null) {
				gameTableCmdService.removeTable(gameTable.getNo());
				gameService.gameTableFinished(Game.daboluo, gameId);
		...
		if ("game start".equals(msg)) {// 游戏开始
			...
			GameTable gameTable = gameService.findTableByGameAndServerGameGameId(Game.daboluo, gameId);
			if (gameTable != null) {
				gameService.gameTablePlaying(Game.daboluo, gameId);
				gameTableMsgService.updateGameTable(gameTable);
			}
		}
		if ("game delay".equals(msg)) {// 游戏延时
			...
			GameTable gameTable = gameService.findTableByGameAndServerGameGameId(Game.daboluo, gameId);
			// 延时1小时
			gameService.delayGameTable(Game.daboluo, gameId, gameTable.getDeadlineTime() + 1 * 60 * 60 * 1000);
		}
	}
}
